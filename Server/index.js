...
app.get('/', (req, res) => {
    res.json({message: "Hi this is a test"})
})
...
app.post('/admin/signup', async (req, res) => {
    ...
    if(foundAdmin){
        ...
        res.status(200).json({message: "Sign up was successful", token, username});
    }
})
...
app.post('/admin/login', async (req, res) => {
    ...
    if(foundAdmin && (await bcrypt.compare(password, foundAdmin.password))){
        const token = generateToken(username);
        res.status(200).json({message: "You are logged in", token, username});
    } else {
